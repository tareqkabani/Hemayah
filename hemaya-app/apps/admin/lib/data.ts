...
export async function getAdminData() {
  ...
  const [listsQ, itemsQ, notifsQ, sysQ, legalsQ, ccrQ, settingsQ, auditQ, healthQ, staffQ, orgQ, holidaysQ] = await Promise.all([
    ...
    rpc("admin_list_org"),
    // تقويم العطل الرسمية — يُغذّى يدوياً لأنّ الأعياد قمريّة ومُدد العطل
    // تُحدَّد بتعميمٍ سنويّ، فلا تُثبَّت في الشيفرة.
    supabase.from("holidays").select("day, name, kind").order("day"),
  ]);
  ...
  return {
    ...
    holidays: (holidaysQ as { data: { day: string; name: string; kind: string }[] | null }).data ?? [],
  };
}
