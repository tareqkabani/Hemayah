...
import { Card, Tag, InlineAlert } from '@/components/ds';
import { SecretCode, DeadlineTimer } from '@/components/patterns';
      ...
      <Screen title="التظلّم أمام النائب العام" lede={filed ? 'تظلّمك قيد النظر — ستُشعَر بالنتيجة فور البتّ.' : isReject ? 'إن رأيت أنّ رفض طلبك غير مبرّر، يمكنك التظلّم على القرار أمام النائب العام.' : 'إن رأيت أنّ أنواع الحماية المقرّرة أو القرار لا يفي بحاجتك، يمكنك التظلّم أمام النائب العام.'}>
        ...
        {filed ? (
          ...
          <Card className="card pad">
            ...
            <button className="btn btn-primary" style={{ width: '100%' }} disabled={!reason.trim()} onClick={() => onFile({ scope, reason: reason.trim() })}>
...
const NAV = [
  ...
  { id: 'messages', t: 'المراسلات', icon: 'forum', C: MessagesDetailed, badge: 2 },
  { id: 'notifications', t: 'الإشعارات', icon: 'notifications', C: NotificationsDetailed, badge: 2 },
];
...
function PortalApp() {
  ...
  const fileGrievance = (g) => setStates((s) => ({ ...s, [selectedReqId]: { ...s[selectedReqId], grievance: { status: 'filed', ref: 'GRV-2026-0192', date: '9 مايو 2026', ...g } } }));
      ...
      <div className="main">
        <header className="topbar">
          ...
          <span className="row" style={{ marginInlineStart: 'auto', gap: 8 }}>
            ...
            <SecretCode code={selReq ? selReq.secret : CASE.secret} canReveal={false} />
          </span>
        </header>
