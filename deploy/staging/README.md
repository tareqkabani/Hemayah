...
> **⚠️ نسخ CLI الأحدث من 2.109 تربط المنافذ على `127.0.0.1` فقط** — فيصل سيرفر القاعدة نفسه إليها ولا يصل غيره، وتظهر «fetch failed» عند الدخول من الواجهات. العلاج المعتمد: انشر stack كشف المنافذ على **Portainer سيرفر القاعدة** — Compose path: `deploy/staging/db-proxy.stack.yml` — **بلا متغيّرات** (العنوان مثبَّت افتراضياً على سيرفر القاعدة الحالي؛ يُتجاوز بـ`DB_BIND_IP` عند الحاجة). تحقّق بعده من أي جهاز: `curl http://<DB_HOST>:55321/auth/v1/health`.
