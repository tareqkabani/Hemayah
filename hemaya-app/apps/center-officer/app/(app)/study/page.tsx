...
export default async function Page() {
  await requireRole("studier" as any, { loginPath: "/login", denyPath: "/403" });

  // حالات الدراسة الفعليّة (RLS: studier_under_study — status='under_study').
  ...
  const [{ data: cases }, { data: mine }] = await Promise.all([
    ...
    supabase.from("studies").select("case_id").eq("studier_id", user?.id ?? ""),
  ]);
  ...
  return <StudyEvalPortal role="studier" initialRows={initialRows} />;
}
