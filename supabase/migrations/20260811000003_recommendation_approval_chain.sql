-- ============================================================
--  سلسلة اعتماد توصية الجهة المختصة — تنفيذ ما جمّده المخطّط ولم يُكتب قطّ
--
--  المخطّط (20260704000001 §«طبقة الفروع وسلسلة الاعتماد») جمّد النموذج:
--  جدولا approval_chains وrecommendation_approvals، وعمود
--  recommendations.approval_status بقيمه الأربع، وسياساتُ RLS تفرّق
--  clerk عن head عن hq عبر cb_level(). لكن **لا دالةَ واحدةً تكتب في أيٍّ
--  منها**: العمود لم يغادر 'preparing' قطّ، والجدولان فارغان منذ إنشائهما.
--  والتدفّق المنفَّذ يرفع التوصية للمركز مباشرةً — موظف الفرع يستدعي
--  record_recommendation فتصير الحالة triage — بلا مرور برئيس الفرع،
--  رغم أن زرّ النموذج نفسه مكتوبٌ عليه «رفع للاعتماد».
--
--  الأثر النظاميّ لا الشكليّ: الردّ الرسميّ للجهة المختصة على إحالة المركز
--  يصدر اليوم بتوقيع موظفٍ واحد. القناة الورقية تُثبت المقصود — الخطاب
--  الوارد بالبريد يصل موقّعاً من الجهة أصلاً، فاعتمادُه تمّ خارج النظام؛
--  والقناة الإلكترونية وحدها هي التي فقدت هذا الركن.
--
--  فهذه المهاجرة تكتب الطرف الناقص، بحدودٍ صريحة:
--   • القناة الإلكترونية (ردّ الفرع على إحالة المركز) تمرّ بالسلسلة إلزاماً.
--   • القناة الورقية تبقى كما هي — الاعتماد سبق ورودَ الخطاب.
--   • submit_entity_recommendation (طلبٌ نيابةً عن الشخص، يُنشئ قضيةً جديدة)
--     **خارج السلسلة**: لا حالةَ «قضية محتجزة» في case_status تحمل الطلبَ
--     ريثما يعتمد الرئيس، وابتكارُها يمسّ الفرز والمستفيد معاً. تُعالَج
--     في مهمّةٍ مستقلة، ولا تُوسَم زوراً بأنها اعتُمدت.
--
--  الميعاد (م7): مهلةُ الدرجة من approval_chains.sla، مقيّدةً بسقف مظلّة
--  الخمسة أيام (recommendations.due_at) — فلا يتجاوزها اعتمادٌ متأخّر.
--  ولا اعتمادَ آليّاً عند التجاوز: تجاوزُ المهلة يُرصد ويُصعَّد للمقر،
--  ولا يُنتِج موافقةً بلا إنسان.
-- ============================================================

-- ── 0) مُعِدّ التوصية: «المُعِدّ» في شاشة الاعتماد يحتاج فاعلاً لا نصّاً ──
...
-- ── 1) سلسلة الاعتماد المفعّلة: درجةٌ واحدة لكل جهة (رئيس الفرع) ──
-- البنية تقبل 1..ن؛ والمفعَّل الآن الدرجة الأولى وحدها كما نصّ المخطّط
-- («الدرجة 1 = رئيس الفرع المباشر (إلزامية دائماً)»).
...
-- ── 2) الكتابة المباشرة على التوصيات مقفلة — كل تغييرٍ عبر دالةٍ محروسة ──
-- rec_branch_rw كانت `for all` مع منحة authenticated=arwd، فكان بوسع موظف
-- الفرع تعديل approval_status أو received_at بطلب PATCH مباشرٍ من المتصفّح
-- ويقفز فوق السلسلة كلّها. القراءة تبقى (السياسات كما هي)، والكتابة تنحصر
-- في دوال SECURITY DEFINER أدناه.
revoke insert, update, delete on public.recommendations        from authenticated;
revoke insert, update, delete on public.recommendation_approvals from authenticated;

-- ── 3) رفع التوصية لاعتماد الرئيس (موظف الفرع) ──
-- تُودَع مسوّدةُ التوصية على التوصية المعلّقة نفسها (لا سجلّ مكرر)، وتُفتح
-- درجةُ اعتمادٍ معلّقة بميعادها. لا تصل المركزَ هنا: received_at يبقى فارغاً
-- وحالة القضية تبقى referred.
...
-- ── 4) بتّ رئيس الفرع: اعتمادٌ يرفع للمركز، أو إعادةٌ للموظف بملاحظة ──
...
-- (أسماء الأعمدة المُخرَجة مسبوقة بـnew_ عمداً: اسمٌ مجرّدٌ مثل case_status
--  يظلّل النوعَ نفسه داخل plpgsql فيكسر 'triage'::case_status.)
...
-- ── 5) قراءة حالة السلسلة للبوابة ──
-- توصياتُ الفرع التي تعيش داخل السلسلة (بانتظار الاعتماد أو معادة)، بكامل
-- ما تحتاجه شاشة المراجعة — لأن اسم المُعِدّ في auth.users وهي محجوبةٌ عن
-- authenticated؛ فالكشف محصورٌ بالفرع (ولجهة المقر إشرافاً) وبالدالة وحدها.
...
-- ── 6) إقفال التجاوز: القناة الإلكترونية لم تعد تصل المركز مباشرةً ──
-- التعريف نفسه (20260809000001) بتغييرٍ واحد: فرع 'electronic' يرفض ويدلّ
-- على السلسلة، بدل أن يقيّد التوصية ويحوّل الحالة إلى triage. الورقية تبقى
-- على حالها بحرفها — الخطاب وصل موقّعاً من الجهة، والاعتماد تمّ خارج النظام.
